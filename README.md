...
The first thing we did before diving into a solution was to understand the problem. Things we understood and considered before arriving at a solution were;

* The salary is an array
* The average should exclude the smallest and largest elements in the array
* If we are excluding min and max the next thing to consider is that the length of the list muse be more than 2

Based on the above the next thing we did was to decompose the problem and we arrived at smaller parts to solve the issue ;
...
##### Solution Steps 
* Step 1:
declare an array of integers to store average salary
Initialize max and min to the first element in the salary array and initialize total to 0
* Step 2
validate that the length of salary is more than 2 and <=100 otherwise return 0.0
* Step 3:
...
call the method to calculate the average and print the result
...
### Approach
Our first thought was to use loops to iterate over the array and square while we iterate over it. After iterating and squaring we sort using an insertion sort algorithm. After this initial solution we iterated on it using functional programming since that will be a concise and fairly efficient way to find a solution. After this decision the next step was to figure out a method that could allow us to index our array and apply some operation to all the elements. This is where the two methods we need come into play map and stream. Where map allows us to apply some operation to data that is given to it from a stream. 
...
#### Solution Steps.
* Step 0: Create a new method that will take parameters for the remainder, two ListNodes and a return ListNode.
...
* Step 3: Inside the branch from step 3 we first find the `sum`, our `sum` is always `ListNode.val + ListNode.val + remainder`. This way we always make sure that the remainder that we passed as parameter to our method is propagated.
...
* Step 9: `This step is only for when there is a remainder and but all the nodes are null`. Here I use an else branch in the if statement that handles the remainders. In the else branch I call a unique recursive statement where l1 is null, l2 is null but remainder is 1 `nodeNumGetter3(null, null, returnList.next, 1)`. This triggers an else branch to handle it pseudocode description below.
```Java
//This is where we handled the recursive call
